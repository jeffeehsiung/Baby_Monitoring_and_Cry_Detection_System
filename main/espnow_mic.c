...
//I2S built-in ADC channel 0 is GPIO1
...
void i2s_adc_capture_task(void* task_param)
{
    ...
    uint8_t* mic_read_buf= (uint8_t*) calloc(READ_BUF_SIZE_BYTES, sizeof(char));
    uint8_t* audio_output_buf = (uint8_t*) calloc(READ_BUF_SIZE_BYTES, sizeof(char));
    ...
    while(true){
        ...
        if (xStreamBufferSend(mic_stream_buf, audio_output_buf, READ_BUF_SIZE_BYTES * sizeof(char), portMAX_DELAY) != bytes_read) {
